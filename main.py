...
def imag_freq_error_window(settings, list_of_filenames, error_message, imag_freq_conf_list):
    ...
    if settings["Mode"] == "Create input files" and not conf_suffixes:
        ...
        Button1 = Button(imag_freq_error_window, text=" Don't proceed ", command=abort_run)
...
def read_settings():
    ...
    default_settings = {
        ...
        "Check for dup confs in XYZ/SDF files": False,
...
To analyse data:
Drag and drop all Gaussian/ORCA output files for
...
def open_about_window():
    ...
    label2.pack()
    label2b = Label(
        window, text="For automated analysis of quantum chemical spectroscopic calculations", font=("Arial", 10)
    )
    label2b.pack()
...
every conformer of your compound here, together.
            ...
            if keep_imag_freq_flag != "proceed without exclusion":
                if not data[50] or var1.get() is True:
                    if multi_out_file_flag == "Multiple output files" and var1.get() is False:
                        ...
                        incompatibility_label.configure(text=incompatibility_text)
                        message_height = 95
                    elif multi_out_file_flag and var1.get() is True and len(out_file_content_types) == 1:
                        incompatibility_frame.grid(row=199, column=1)
                        incompatibility_label.pack()
                        incompatibility_label.configure(font=("Segoe UI", 9))
                        incompatibility_text = (
                            ...
                            "conformers in\nseparate files. Calculated results from these new input "
                            "files will be incompatible for combined analysis later with\nthe current "
                            ".out\.log file(s). This is because conformer data can not be matched across "
                            "these new and old files with different\nconformer arrangements during "
